const codeFlower = (graph) =>  {                                // the function d3.json receives another callback that contains an error variable. 'graph' is the graph we've been referring to.
    ...
    let simulation = d3.forceSimulation()                             // call the function forceSimulation that creates the simulation object
        .force('center', d3.forceCenter(width / 2.5, height / 2))         // 2. create forces. put the nodes (x, y) in the middle of the screen (half its width and height)
    ...
    //     )                                            // *** 4. the nodes repel: the higher the negative value (of force), the further they repel.
        ...
        const circles = node.append('circle')
            ...
            //     }
                if (['Female'].includes(d.id.split(' ')[0])) {
                    // return '#ffff00';
                    return '#3FB8AF';    //happy mom
                    // return '#004358';   // vitamin c
                }
                else if (['Adult'].includes(d.id.split(' ')[0])) {
                    // return '#e75480';
                    return '#7FC7AF';    //happy mom
                    // return '#1F8A70';   // vitamin c
                }
                else if (['MMR'].includes(d.id.split(' ')[0])) {
                    // return '#000000';
                    return '#FFF7A1';    //happy mom
                    // return '#BEDB39';   // vitamin c
                }
                else if (['Gross'].includes(d.id.split(' ')[0])) {
                    // return '#32CC32';
                    return '#FF9E9D';    //happy mom
                    // return '#FFE11A';   // vitamin c
                }
                else if (['Adolescent'].includes(d.id.split(' ')[0])) {
                    // return '#33ff67';
                    return '#FF3D7F';    //happy mom
                    // return '#FD7400';   // vitamin c
                }
                else {
                    return '#red';
                }
        ...
        const labels = node.append('text')
            .text(function (d) {
                if (['Female'].includes(d.id.split(' ')[0])) {
                    return (
                        'Youth ' + d.id.slice(0, 16) + 'Rate ' + 
                        d.r + '%');
                }
                else if (['Adult'].includes(d.id.split(' ')[0])) {
                    return (
                        d.id.slice(0, 6) + 'Female Literacy Rate ' + 
                        d.r + '%');
                }
                else if (['Gross'].includes(d.id.split(' ')[0])) {
                    return (
                    ...
                    return (
                        d.id.slice(0, 21) + 'Rate ' + 
                        d.r + '%');
                }
                else if (['MMR'].includes(d.id.split(' ')[0])) {
                    return (
                        'Maternal Mortality Rate (' + d.r + '/100000 live births)')
                }
                else {
                    return d.id;
                }
